#!/usr/bin/env python3
"""
Update AUR PKGBUILD from GitHub releases.
...
PKGBUILD template and generates the .SRCINFO file.
"""
...
def load_config(name: str, script_dir: Path) -> Dict:
    ...
    required_keys = ['github_repo', 'template_path', 'submodule_path', 'asset_patterns']
...
def main():
    """Main function to update the AUR PKGBUILD."""
    # Parse command-line arguments
    parser = argparse.ArgumentParser(
        description="Update AUR PKGBUILD from GitHub releases"
    )
    parser.add_argument(
        "name",
    ...
    submodule_path = script_dir / "../" / config['submodule_path']
    ...
    # Write output
    pkgbuild_path = submodule_path / "PKGBUILD"
    pkgbuild_path.write_text(rendered)
    print(f"Successfully updated {pkgbuild_path}")
    print(f"\nPKGBUILD updated to version {version}")

    # Generate .SRCINFO and overwrite the existing file
    print(f"\nGenerating .SRCINFO from {pkgbuild_path}...")
    srcinfo_path = submodule_path / ".SRCINFO"
    with open(srcinfo_path, "w", encoding="utf-8") as srcinfo_file:
        subprocess.run(
    ...
    print(f"Successfully generated .SRCINFO")
